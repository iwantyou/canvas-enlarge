...
export type BaseTarget<T> = HTMLElement | Element | Window | Document | RefObject<T> 
export type Target = BaseTarget<HTMLElement | Element | Window | Document>
const getTarget = (target:Target | undefined) => {
  if(!target) return;
...
function useEventListener(eventName: string, handler: Function, options: Options = {}) {
  ...
  useEffect(() => {
    const targetElement = getTarget(options.target!) || window;
